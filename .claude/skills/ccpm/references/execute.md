...
Get issue details: `gh api repos/$REPO/issues/<N> --jq '{title,body,labels:[.labels[].name]}'`
...
### Preflight
1. Verify issue exists and is open: `gh api repos/$REPO/issues/<N> --jq '{state,title,body,labels:[.labels[].name]}'`
...
jq -n --arg u "$me" '{assignees:[$u]}' > /tmp/assignee.json
gh api --method POST "repos/$REPO/issues/<N>/assignees" --input /tmp/assignee.json --jq .number
jq -n '{labels:["in-progress"]}' > /tmp/label.json
gh api --method POST "repos/$REPO/issues/<N>/labels" --input /tmp/label.json --jq '[.[].name]' 
```
