...
> **この文書について。** 複数セッションにまたがる調査を 1 本にまとめた正典である。検証の途中で 2 度の訂正（GitHub 到達性の原因、push 制限の内容）が入っており、**訂正結果は本文に織り込み済み**。訂正の経緯そのものは[付録 A](#付録-a-検証履歴と訂正の記録)に、各主張の出典と裏付け／反証の突き合わせは[`docs/ccpm-evidence-review.md`](./ccpm-evidence-review.md) に分離した。
...
3. **「CCPM がクラウドで動かない」のではない。4 つの層のうち 2 つが書き換えを要する。**
   ...
   | ライフサイクル終端（epic-merge・ブランチ整理） | **部分的に書き換え** — ブランチ削除は確実に不可。main へのマージ push は**ブロックが観測されない** |
...
**(A) main への push — ブロックは観測されなかった【訂正】［実測］。**

初版は [#56474](https://github.com/anthropics/claude-code/issues/56474)（harness レベルの恒久ブロック、設定でも解除不可）を根拠に「main へは push できない」としていた。**これを実測で確かめたところ、支持されなかった。**

main を変更せずにどの層が拒否するかを切り分けるため、**親を持たない孤立コミット**を作って main へ push した。非 fast-forward なので、プロキシを通過しても GitHub 側で必ず弾かれ、main は一切変わらない。

```
$ ORPHAN=$(git commit-tree $(git hash-object -t tree /dev/null) -m probe)
$ git push origin "$ORPHAN:refs/heads/main"
 ! [rejected]  4064546... -> main (non-fast-forward)
hint: Updates were rejected because a pushed branch tip is behind its remote counterpart.
```

**返ってきたのは 403 ではなく git の non-fast-forward 拒否であり、リクエストは GitHub まで到達している。** 対照として同一セッションで直後にブランチ削除を試すと変わらず 403 が返るため、**ゲートが適用される操作では実際に 403 で止まる**。main で 403 が出なかったことには意味がある。

harness 側も確認した。#56474 が報告する素の形 `git push origin main` を実行しても遮断されず、そのまま git に渡った（ローカル main は remote と同一のため `Everything up-to-date`）。**文字列レベルのブロックも観測されない。**

| 確認項目 | 結果 |
| --- | --- |
| プロキシによる main 宛 push のブロック | **観測されない**（GitHub まで到達） |
| harness による `git push origin main` の遮断 | **観測されない** |
| プロキシのゲート自体 | **生きている**（削除は同一セッションで 403） |
| fast-forward の実 push が通るか | **未実施** |

**結論:** 「恒久ブロックで解除手段がない」という記述は本サーフェスでは支持されない。ただし「通る」と断言するには fast-forward を実際に通すしかなく、それは main を進めてしまう。**巻き戻しには force-push が要り、削除が塞がれている以上それも塞がれている可能性が高い**ため、実施していない。

**CCPM への含意:** `epic-merge` の main へのマージ push は、**書き換え必須とは言えない**。PR ベースへの置換は依然として推奨だが（レビュー可能性とロールバックの容易さのため）、技術的な強制ではない。
...
| CI 追従 | **クラウド** | Auto-fix |
| `epic-merge` | **クラウド可（PR 経由を推奨）** | main 直 push のブロックは観測されない（3.5）。PR 化は運用判断 |
...
- **クラウドで書き換えが確実に要るのは GitHub 同期の 1 層と、ブランチ整理だけ。** スクリプト層と並列実行層はそのまま動き、`epic-merge` の main push もブロックが観測されない。当初「4 層のうち 2 層」と見積もっていたが、実質は **1.5 層**に近い。
...
| 統合時 | main への push は harness レベルで恒久ブロック（根拠: 外部 Issue のみ） | **取り下げ** — 孤立コミットによる切り分けで、プロキシも harness もブロックしていないと判明（3.5 A） |
