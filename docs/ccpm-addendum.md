...
> **この文書について。** 複数セッションにまたがる調査を 1 本にまとめた**検証の記録**である。検証の途中で **10 件の訂正**（過大評価 6・過小評価 4）が入っており、**訂正結果は本文に織り込み済み**。訂正の経緯そのものは[付録 A](#付録-a-検証履歴と訂正の記録)に、各主張の出典と裏付け／反証の突き合わせは[`docs/ccpm-evidence-review.md`](./ccpm-evidence-review.md) に分離した。

> **⚠️ 現時点の推奨は本書ではなく [`docs/ccpm-cloud-guide.md`](./ccpm-cloud-guide.md) を参照すること。** 本書の第 4〜5 章は「CCPM の `gh` 呼び出しを `gh api` の REST に書き換える」ことを推奨として書いているが、**この推奨は一度も実証されていない**。前提となる `gh api repos/{owner}/{repo}` の 200 応答を本調査では一度も観測しておらず、REST 実装スクリプトも成功していない。CCPM 自体も一度も起動していない。ガイド側では、実証済みの経路（ローカル同期＋クラウド実行、GitHub 操作は MCP）を推奨に据え、REST 書き換えは「できたらよいこと」として付録に降格してある。**本書は環境の測定結果の記録として読み、設計判断にはガイドを使うこと。**
...
3. **「CCPM がクラウドで動かない」のではない。ただし書き換え箇所は当初の想定より広く、`init.sh` を含む 2 層に及ぶ。**

   | 層 | クラウドでの状態 |
   | --- | --- |
   | 報告系スクリプト（status / standup / 検索） | **無改修で動く**（純ローカルであることをソースで確認） |
   | 初期化（`init.sh`） | **書き換え必須** — `gh repo view` / `gh label create` / 対話的 `gh auth login` に依存 |
   | 並列実行（worktree・ブランチ操作） | **プロキシ由来の制約は無い**。ただし CCPM は 1 epic ブランチに複数エージェントが同時コミットする設計で、**競合とリベースの安定性は未検証** |
   | GitHub 同期（epic-sync / issue-sync） | **書き換え必須** — `gh` 高レベルは事実上すべて不可。`gh api` REST へ。かつ到達性がサーフェス依存 |
   | ライフサイクル終端（epic-merge・ブランチ整理） | **ブランチ削除のみ不可**。main への push は force-push まで通る（**ただし保護なしの main で検証**） |
...
| `gh issue view` | **403（GraphQL pinned-set）** | ［実測］ |
| `gh issue close` | **403（GraphQL pinned-set）** | ［実測］`-R` を付けても同じ |
| `gh issue create` | **403（GraphQL pinned-set）** | ［実測］`RepositoryInfo` preamble で止まる |
| `gh pr list` | **403（GraphQL pinned-set）** | ［実測］**PR 系ですら通らない** |
| `gh api` + REST パス | **許可されている経路** | ［公式］プロキシのエラー文自身が案内している |

**「pinned set は PR 用」という公式の説明は、`gh` のクエリを含まない。** `gh pr list` すら `PullRequestList` として名指しで拒否される。プロキシが通すのは Anthropic 側の PR レビュー機能が使う特定の操作だけで、**`gh` の高レベルサブコマンドは事実上すべて不可**と考えてよい。当初の「軒並み」という表現は正しかった。

**さらに `init.sh` がこのサーフェスでは完走しない［実測・外部］。** CCPM の初期化スクリプトは `gh auth status` / `gh auth login`（対話） / `gh repo view` / `gh label create` / `gh label list` / `gh extension install` に依存する。`gh repo view` は GraphQL で確実に落ち、`gh auth login` はそもそも対話的でクラウドでは実行できない。**CCPM は導入の第一歩から書き換えが要る。**
...
**したがって push は制約にならない。** 「1 タスク = 1 クラウドセッション」への読み替えは**必須ではなくなった**。むしろ後者は 3.7 のレート上限に当たるため、既定は worktree 並列でよい。

> **ただし検証対象がずれていた点を明記しておく。** CCPM の実際の並列モデルは「タスクごとに別ブランチ」ではない。`references/execute.md` は **1 つの `epic/<name>` ブランチに複数エージェントが同時にコミットする**設計で、各エージェントは `git pull --rebase origin epic/<name>` で同期する。上の実測は「複数ブランチへ push できるか」を測ったもので、**CCPM が実際に要求するのは同一ブランチへの並行 push とリベース**である。プロキシ由来の制約が無いことは言えるが、**競合とリベースの安定性は未検証**であり、そちらが本当のリスクになる。なお CCPM 自身が execute.md で "Never use --force on git operations" を規定しているため、force-push が通ることは CCPM 的には無関係である。
...
> **重要な限定。** 検証したリポジトリの main は **`protected: false`**（`list_branches` で確認済み）である。公式の規則は保護ブランチへの push を拒否すると明記しているため、**この結果は保護なしのデフォルトブランチに限る。** 実運用で main を保護しているなら `epic-merge` の直接 push は通らない。「epic-merge はクラウド可」はその前提付きで読む必要がある。
...
- **`.github/workflows/` 配下は push できる［実測］。** ワークフローファイルを含むコミットの push が成功した（`c387d89..e47e981`）。[#61189](https://github.com/anthropics/claude-code/issues/61189) の報告は本サーフェスでは再現しない。**これは同時に、Actions 経由でクラウドから間接的にブランチを削除できる可能性を意味する**（未検証・3.5 参照）
...
- **クラウドで書き換えが要るのは、GitHub に触る 2 層（`init.sh` と同期処理）とブランチ削除。** 報告系スクリプト・並列実行・`epic-merge` はそのまま動く。見積もりは「2 層 → 1 層」と縮んだ後、自己監査で `init.sh` が加わって再び 2 層に戻った。
...
| 統合時 | `.github/workflows/` へは push できない（根拠: 外部 Issue のみ） | **取り下げ** — 実測で push 成功（3.8） |
| 統合時 | 「`gh` は軒並み GraphQL」は言い過ぎで、create / comment / close は未検証 | **差し戻し** — `issue view` / `close` / `create` / `pr list` すべて GraphQL 403。**元の「軒並み」が正しかった**（3.2） |
| 統合時 | 決定的スクリプト層は GitHub API に触らないので無改修で動く | **限定が必要** — `status.sh` / `standup.sh` は純ローカルだが、**`init.sh` は `gh` に全面依存**しており完走しない（3.2） |
| 統合時 | main への push が通る | **限定が必要** — 検証したのは `protected: false` の main のみ。保護ブランチでは公式規則により拒否される（3.5 A） |
| 統合時 | worktree 並列モデルの push を検証した | **対象がずれていた** — CCPM は 1 epic ブランチに複数エージェントが同時コミットする設計。競合とリベースの安定性は未検証（3.4） |
