...
3. **「CCPM がクラウドで動かない」のではない。4 つの層のうち 2 つが書き換えを要する。**
   ...
   | ライフサイクル終端（epic-merge・ブランチ整理） | **書き換え必須** — main push 不可、ブランチ削除も不可 |
...
**(A) main への push は harness レベルで恒久ブロック［外部・未検証］。** [#56474](https://github.com/anthropics/claude-code/issues/56474) によれば `settings.json` でも環境変数でも解除できない。本調査では実 push を行っていないため根拠は外部情報のみ（`--dry-run` は ref 更新コマンドを送らないため判定材料にならない）。
...
**対処:** `epic-merge` を「main への直接マージ」から「PR 作成 → 人間または Auto-fix がマージ」に置き換える。ブランチ整理は GitHub UI・ローカル・GitHub Actions のいずれかへ逃がす。**これは CCPM の仕様変更に相当するため、導入前に決めておく必要がある。**
...
| CI 追従 | **クラウド** | Auto-fix |
| `epic-merge` | **PR 経由へ置換** | main 直 push が不可（3.5）。クラウドは PR 作成まで |
| ブランチ整理 | **クラウド外** | 削除がどの経路でも不可（3.5） |
...
- **`epic-merge` を PR ベースに置き換える**（3.5）。ここが最も設計変更に近い
...
- **クラウドで書き換えが要るのは 2 層だけ。** GitHub 同期（`gh` 高レベル → `gh api` REST）と、ライフサイクル終端（`epic-merge` → PR、ブランチ整理は外へ）。スクリプト層と並列実行層はそのまま動く。
...
CCPM 導入後の役割分担は、前回の結論をより鮮明にする。**ローカルは「仕様を決める場所」、クラウドは「仕様を実行する場所」、GitHub Issues は「両者をつなぐ唯一の真実」。** そこに 1 つ足すなら、**「マージとブランチ整理はクラウドの外」**である。
...
**訂正が 2 度とも「制約を過大に見積もっていた」方向だった点は記録しておく価値がある。** エラーメッセージ（"An org admin must connect the Claude GitHub App"）が実態と違うこと、公式ドキュメントを当たる前に実測から一般化したことが原因だった。
