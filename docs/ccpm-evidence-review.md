...
| 5a | worktree 並列は push 制限で成立しない | **反証（実測で決着）** — 名前を問わず複数ブランチへ push できた | **仮説を取り下げ。ただし削除は不可という別の制約が判明** |
...
本レポートの初版公開後、同一セッションから 7 通りの push を実際に試した（詳細は [addendum 3.11](./ccpm-addendum.md)）。
...
**1 セッション・1 worktree から、名前を問わず複数ブランチへ push できる。** `claude/` プレフィックスすら必須ではなかった。公式ドキュメントの規則（`claude/` は常に許可、それ以外は 3 条件のいずれかに該当する場合のみ拒否）と整合する結果である。

#### 判定

**仮説は取り下げ。** CCPM の epic worktree が複数タスクブランチを push する構造は、そのままクラウドで動く。残る制約は 2 つに絞られた。

- **`epic-merge`（main への直接マージ push）は不可** — worktree 並列とは別の話であり、こちらが本質的な壁
- **ブランチ削除が一律 403** — 後片付けがクラウドから実行できない（次章 3.5）
...
### 3.5 ブランチ削除だけが一律に拒否される【新規・実測】

push 実験の後片付けで判明した。**作成と更新は通るのに、削除だけが 403 で拒否される。**
...
| # | 問い | 検証方法 | コスト |
| --- | --- | --- | --- |
| ~~1~~ | ~~1 セッションから複数ブランチを push できるか~~ | **実測で決着（仮説 5 参照）。名前を問わず push 可能** | 完了 |
| 2 | `POST .../sub_issues` は Claude のプロキシ経由 REST でも通るか | API 直接経路が有効なサーフェス、またはローカルで `ccpm-subissue-rest.sh experiment` | 1 分。**現時点で最優先** |
...
| 6 | main への push は本当にブロックされるか | 検証用リポジトリで実 push | 5 分。`--dry-run` は判定材料にならない（ref 更新コマンドを送らないため） |
...
3. **代わりに、CCPM のライフサイクル終端に 2 つの壁があることが判明した。** `epic-merge` の main push は解除手段のない恒久ブロックであり、さらに**ブランチ削除が一律 403 で拒否される**（作成と更新は通る）。epic 完了処理は、マージも後片付けもクラウドの外に出す必要がある。
...
| ライフサイクル終端（epic-merge / ブランチ整理） | **書き換え必須**（main push 不可 → PR 経由、ブランチ削除も不可） |
