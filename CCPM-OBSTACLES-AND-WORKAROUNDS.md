...
> **訂正 (2026-08-11 追記)**
> 本レポートの A 章は「REST も 403 で `gh` は GitHub API に到達できない」と
> 記載しているが、**これは誤り**。その後の網羅検証で **REST は全面的に利用可能**
> であることが判明した。遮断されているのは GraphQL のみであり、CCPM が使う
> porcelain コマンドがすべて GraphQL を叩くため落ちていた。
> B 章の「代替なし」とした `gh-sub-issue` も、ネイティブの REST サブイシュー API
> で代替可能。正確な可否は `GH-CAPABILITY-MATRIX.md` を参照。
> **結論として、MCP への迂回は必須ではなく、CCPM 側を `gh api` に寄せる選択肢が
> あった。**

---

## A. 環境起因の障害 — `gh` の porcelain が GitHub API に到達できない

以下 6 件はすべて同一の根本原因。バイナリは正常で、**GraphQL 経路だけが遮断**
されている (当時は REST も不可と誤認していた。上記の訂正を参照)。
